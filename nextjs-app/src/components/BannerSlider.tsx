 ...
 export default function BannerSlider() {
   ...
   const [tienda, setTienda] = useState({ nombre: 'URBAN', lema: 'Streetwear — drops — fits' })
   ...
   async function cargarConfig() {
     try {
       ...
       if (parsed.length === 0) {
         const hero = typeof data.hero_banner_url === 'string' ? data.hero_banner_url : ''
         setBanners(
           hero
         ...
         setBanners(parsed.filter((b) => b && typeof b.url === 'string' && b.url.trim().length))
       }
     } catch {
       setBanners([
         {
   ...
   if (loading) return <div className="w-full h-[320px] md:h-[55vh] lg:h-[65vh] bg-black animate-pulse" />
   ...
   return (
     ...
     >
       ...
       <div className="absolute inset-0">
         {banners.map((b, i) => {
           const raw = b.link || ''
           const safeLink = typeof raw === 'string' && raw.startsWith('/') ? raw : sanitizeURL(raw)
           return (
             <div
               key={i}
             ...
             >
               ...
               {safeLink && <a href={safeLink} className="absolute inset-0 z-10" />}
             </div>
           )
         })}
       </div>
       ...
       <div className="relative z-20 h-full max-w-[1400px] mx-auto px-6 md:px-10 flex items-center">
         <div className="w-full md:w-[60%] space-y-4">
           <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2">
             ...
             <span className="text-[10px] font-black uppercase tracking-[0.35em] text-white/70">Envío rápido</span>
           </div>
           ...
           <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 z-30">
             {banners.map((_, i) => (
               <button
                 ...
                 className={`h-1.5 rounded-full transition-all ${i === current ? 'w-10 bg-white' : 'w-2 bg-white/30'}`}
               />
             ))}
           </div>
         </>
