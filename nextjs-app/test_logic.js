...
async function runTests() {
    ...
    console.log('Case 1 (Valid ID):', res.normalizadoCategoriaId === 'cat1' ? 'PASS' : 'FAIL', res);

    // Case 2: ID not in list (should preserve ID)
    res = await handleSaveProductLogic({ categoria_id: 'cat999' }, mockCats);
    console.log('Case 2 (ID not found):', res.normalizadoCategoriaId === 'cat999' ? 'PASS' : 'FAIL', res);

    // Case 3: Empty string (should be null)
    res = await handleSaveProductLogic({ categoria_id: '' }, mockCats);
    console.log('Case 3 (Empty):', res.normalizadoCategoriaId === null ? 'PASS' : 'FAIL', res);

    // Case 4: "Seleccionar..." (should be null)
    res = await handleSaveProductLogic({ categoria_id: 'Seleccionar...' }, mockCats);
    console.log('Case 4 (Seleccionar...):', res.normalizadoCategoriaId === null ? 'PASS' : 'FAIL', res);

    // Case 5: Empty categories list (should preserve ID)
    res = await handleSaveProductLogic({ categoria_id: 'cat1' }, []);
    console.log('Case 5 (Empty Cats):', res.normalizadoCategoriaId === 'cat1' ? 'PASS' : 'FAIL', res);

    // Case 6: API failure (simulated by passing null as mockCats, catch block handles it?)
    // Wait, if obtenerTodas throws, we need to mock that.
}
